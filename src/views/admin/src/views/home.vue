...
<div class="content" :style='{"padding":"30px"}'>
	...
	<div :style='{"width":"100%","margin":"0 0 20px 0","alignItems":"center","flexWrap":"wrap","justifyContent":"center","display":"flex"}'>
		<div :style='{"border":" 1px solid rgba(167, 180, 201,.3)    ","margin":"0 10px","borderRadius":"4px","background":"#fff","display":"flex"}' v-if="isAuth('menpiaodingdan','首页总数')">
			...
			<div :style='{"width":"120px","alignItems":"center","flexDirection":"column","justifyContent":"center","display":"flex","order":"1"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"20px","fontWeight":"bold","height":"24px"}'>{{menpiaodingdanCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"14px","color":" #a3b1c9","height":"24px"}'>门票订单总数</div>
			</div>
		</div>
		<div :style='{"border":"1px solid rgba(167, 180, 201,.3)  ","margin":"0 10px","borderRadius":"4px","background":"#fff","display":"flex"}' v-if="isAuth('jiudiandingdan','首页总数')">
			<div :style='{"alignItems":"center","background":"#00e682","display":"flex","width":"80px","justifyContent":"center","height":"80px","order":"2"}'>
				<span class="icon iconfont icon-xihuan" :style='{"color":"#fff","fontSize":"24px"}'></span>
			</div>
			<div :style='{"width":"120px","alignItems":"center","flexDirection":"column","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"20px","color":"#374254","fontWeight":"bold","height":"24px"}'>{{jiudiandingdanCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"14px","color":" #a3b1c9","height":"24px"}'>酒店订单总数</div>
			</div>
		</div>
	</div>
	<!-- statis -->
	...
	<div class="type2" :style='{"alignContent":"flex-start","padding":"0","flexWrap":"wrap","background":"none","display":"flex","width":"100%","position":"relative","justifyContent":"space-between","height":"auto"}'>
		<div id="menpiaodingdanChart1" class="echarts1" v-if="isAuth('menpiaodingdan','首页统计')"></div>
		<div id="jiudiandingdanChart1" class="echarts2" v-if="isAuth('jiudiandingdan','首页统计')"></div>
	</div>
</div>
</template>
<script>
...
export default {
	data() {
		return {
            menpiaodingdanCount: 0,
            jiudiandingdanCount: 0,
	...
	mounted(){
		this.init();
		this.getmenpiaodingdanCount();
		this.menpiaodingdanChat1();
		this.getjiudiandingdanCount();
		this.jiudiandingdanChat1();
	},
	methods:{
		// 词云
		...
		getmenpiaodingdanCount() {
			this.$http({
				url: `menpiaodingdan/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.menpiaodingdanCount = data.data
				}
			})
		},
		menpiaodingdanChat1() {
        ...
        var menpiaodingdanChart1 = echarts.init(document.getElementById("menpiaodingdanChart1"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                menpiaodingdanChart1.setOption(option);
				// 动画效果
				this.myChartInterval(1, option.xAxis.data, option.series[0].data, menpiaodingdanChart1)
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    menpiaodingdanChart1.resize();
                };
            }
        });
      })
		...
		getjiudiandingdanCount() {
			this.$http({
				url: `jiudiandingdan/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.jiudiandingdanCount = data.data
				}
			})
		},
		jiudiandingdanChat1() {
        ...
        var jiudiandingdanChart1 = echarts.init(document.getElementById("jiudiandingdanChart1"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                jiudiandingdanChart1.setOption(option);
				// 动画效果
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    jiudiandingdanChart1.resize();
                };
            }
        });
      })
