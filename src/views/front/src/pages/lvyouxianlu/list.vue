...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"row-gap":"10px","padding":"24px 0","alignItems":"center","flexWrap":"wrap","display":"flex","width":"100%","height":"auto"}'>
      <el-form-item :style='{"margin":"0 10px"}'>
        ...
        <el-input v-model="formSearch.xianlumingcheng" placeholder="线路名称" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
	  ...
	  <el-button v-if="btnAuth('lvyouxianlu','新增')" :style='{"cursor":"pointer","border":"1px solid","padding":"0px 15px","margin":"0 10px 0 0","outline":"none","color":"#000","borderRadius":"4px","background":"none","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/lvyouxianluAdd')"><i v-if="true" :style='{"color":"#000","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
		...
		<div class="list3 index-pv1" :style='{"width":"100%","padding":"30px 0 0","flexWrap":"wrap","background":"#ecf2ea","display":"flex","height":"auto"}'>
		  <div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
		    <div :style='{"border":"0px solid red","width":"200px","padding":"0px","overflow":"hidden","height":"200px"}' class="img-box">
				<img @click.stop="imgPreView(item.luxiantupian)" :style='{"width":"100%","objectFit":"cover","display":"block","height":"100%"}' v-if="item.luxiantupian && item.luxiantupian.substr(0,4)=='http'" :src="item.luxiantupian.split(',')[0]" class="image" />
				<img @click.stop="imgPreView(baseUrl + (item.luxiantupian?item.luxiantupian.split(',')[0]:''))" :style='{"width":"100%","objectFit":"cover","display":"block","height":"100%"}' v-else :src="baseUrl + (item.luxiantupian?item.luxiantupian.split(',')[0]:'')" class="image" />
			</div>
		    <div :style='{"padding":"0 10px","overflow":"hidden","flex":"1","display":"flex","height":"auto"}' class="item-info">
		      <div :style='{"width":"50%","padding":"0 10px 0 0","color":"#999","flex":"1"}'>
				<div class="title">线路名称:{{item.xianlumingcheng}}</div>
		      ...
		      <div class="desc" v-html="item.tujingluduan"></div>
		    </div>
		  </div>
		</div>
	</div>
...
<script>
  export default {
    ...
    created() {
	    ...
	    this.jiaotongfangshiOptions = '飞机,火车,汽车,骑行,徒步'.split(',');
		this.selectOptionsList.push({name:'交通方式',list:this.jiaotongfangshiOptions,tableName: 'jiaotongfangshi',check: -1})
	    this.chuxingshijianOptions = '一月,二月,三月,四月,五月,六月,七月,八月,九月,十月,十一月,十二月,新年'.split(',');
		this.selectOptionsList.push({name:'出行时间',list:this.chuxingshijianOptions,tableName: 'chuxingshijian',check: -1})
      this.getFenlei();
      this.getList(1, '全部');
    },
    //方法集合
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        if (this.formSearch.jiaotongfangshi != '') searchWhere.jiaotongfangshi = this.formSearch.jiaotongfangshi;
        if (this.formSearch.chuxingshijian != '') searchWhere.chuxingshijian = this.formSearch.chuxingshijian;
			let user = JSON.parse(localStorage.getItem('sessionForm'))
		if (this.sortType) searchWhere.sort = this.sortType
		if (this.sortOrder) searchWhere.order = this.sortOrder
        this.$http.get(`lvyouxianlu/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/lvyouxianluDetail', query: params});
      },
