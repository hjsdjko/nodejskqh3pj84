...
<div :style='{"width":"100%","padding":"20px 10%","margin":"30px auto","position":"relative"}'>
    <el-form
      ...
      label-width="auto"
    >
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","float":"left","display":"inline-block"}' label="线路名称" prop="xianlumingcheng">
            <el-input v-model="ruleForm.xianlumingcheng" 
                placeholder="线路名称" clearable :disabled=" false  ||ro.xianlumingcheng"></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","float":"left","display":"inline-block"}' label="路线图片" v-if="type!='cross' || (type=='cross' && !ro.luxiantupian)" prop="luxiantupian">
            <file-upload
            tip="点击上传路线图片"
            action="file/upload"
            :limit="3"
            ...
            <el-form-item :style='{"padding":"10px","margin":"0 0 10px","float":"left","display":"inline-block"}' class="upload" v-else label="路线图片" prop="luxiantupian">
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","float":"left","display":"inline-block"}'  label="交通方式" prop="jiaotongfangshi">
            <el-select v-model="ruleForm.jiaotongfangshi" placeholder="请选择交通方式" :disabled=" false  ||ro.jiaotongfangshi" >
              <el-option
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","float":"left","display":"inline-block"}'  label="出行时间" prop="chuxingshijian">
            <el-select v-model="ruleForm.chuxingshijian" placeholder="请选择出行时间" :disabled=" false  ||ro.chuxingshijian" >
              <el-option
                  v-for="(item,index) in chuxingshijianOptions"
                  :key="index"
                  :label="item"
                  :value="item">
              </el-option>
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","float":"left","display":"inline-block"}' label="途径路段" prop="tujingluduan">
            <el-input
              type="textarea"
              :rows="8"
              placeholder="途径路段"
              v-model="ruleForm.tujingluduan">
            </el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","float":"left","display":"inline-block"}' label="路线详情" prop="luxianxiangqing">
            <editor 
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`lvyouxianlu/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('lvyouxianlu/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`lvyouxianlu/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
