...
<div>
	<div :style='{"width":"100%","padding":"16px calc(50% - 600px)","background":"#ecf2ea"}' class="breadcrumb-preview">
		<el-breadcrumb :separator="'/'" :style='{"fontSize":"14px","lineHeight":"1"}'>
			...
			<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" to="/index/menpiaodingdan"><a>{{item.name}}</a></el-breadcrumb-item>
	...
	<div class="detail-preview" :style='{"width":"100%","padding":"0 10% 30px","margin":"30px auto","position":"relative","flexWrap":"wrap","display":"flex"}'>
		<div class="attr" :style='{"width":"calc(100% - 500px)","margin":"0 0 0 30px","position":"relative","background":"#fff","display":"flex","order":"2"}'>
			...
			<div class="info" :style='{"width":"100%"}'>
				...
				<div class="item" :style='{"padding":"0 0 4px","margin":"0 0 5px 0","borderColor":"#f6f6f6","flexWrap":"wrap","borderWidth":"0 0 1px","display":"flex","borderStyle":"solid"}'>
					<div class="lable" :style='{"margin":"0 5px 0 0","color":"#000000","textAlign":"left","width":"auto","fontSize":"14px","lineHeight":"24px","height":"auto"}'>预订日期</div>
					<div  :style='{"padding":"0px 10px 0","fontSize":"14px","lineHeight":"24px","color":"#333","flex":"1","height":"auto"}'>{{detail.yudingriqi}}</div>
				</div>
				...
				<div class="btn" :style='{"row-gap":"20px","width":"470px","padding":"10px 0","flexWrap":"wrap","display":"block"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 10px 0","outline":"none","color":"#fff","borderRadius":"4px","background":"#598969","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('menpiaodingdan','修改')" @click="editClick">修改</el-button>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 10px 0","color":"#fff","borderRadius":"4px","background":"#00AA3A","display":"inline-block","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('menpiaodingdan','删除')" @click="delClick">删除</el-button>
					<!-- hasChat 无 -->
					<el-button v-if="centerType&&(detail.ispay=='未支付'||!detail.ispay)&&btnAuth('menpiaodingdan','支付')" :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 10px 0","outline":"none","color":"#fff","borderRadius":"4px","background":"#598969","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' type="warning" size="small" @click="payClick">支付</el-button>
				</div>
			</div>
		</div>
...
<script>
  ...
  export default {
    ...
    methods: {
		...
		payClick(){
			localStorage.setItem('paytable','menpiaodingdan')
      ...
      nextClick(page) {
		...
		// 修改
		editClick(){
			this.$router.push(`/index/menpiaodingdanAdd?type=edit&&id=${this.detail.id}`);
		},
		// 删除
		delClick(){
			this.$confirm('是否删除此门票订单？')
			  .then(_ => {
			    this.$http.post('menpiaodingdan/delete', [this.detail.id]).then(res => {
			      if (res.data.code == 0) {
			        this.$message({
			          type: 'success',
			          message: '删除成功!',
