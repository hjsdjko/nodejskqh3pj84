...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"row-gap":"10px","padding":"24px 0","alignItems":"center","flexWrap":"wrap","display":"flex","width":"100%","height":"auto"}'>
      ...
      <el-form-item :style='{"margin":"0 10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>城市分类：</div>
        <el-input v-model="formSearch.chengshifenlei" placeholder="城市分类" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>星级：</div>
        <el-input v-model="formSearch.xingji" placeholder="星级" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>退房时间：</div>
        <el-date-picker 
          v-model="timeRange"
          type="daterange"
		  :style='{"border":"1px solid","padding":"3px 10px","outline":"none","borderRadius":"8px","background":"#fff","width":"auto","justifyContent":"center"}'
          range-separator="至"
          start-placeholder="退房时间起始"
          end-placeholder="退房时间结束"
	  ...
	  <el-button v-if="btnAuth('jiudiandingdan','新增')" :style='{"cursor":"pointer","border":"1px solid","padding":"0px 15px","margin":"0 10px 0 0","outline":"none","color":"#000","borderRadius":"4px","background":"none","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/jiudiandingdanAdd')"><i v-if="true" :style='{"color":"#000","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
		...
		<div class="list3 index-pv1" :style='{"width":"100%","padding":"30px 0 0","flexWrap":"wrap","background":"#ecf2ea","display":"flex","height":"auto"}'>
		  <div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
		    <div :style='{"border":"0px solid red","width":"200px","padding":"0px","overflow":"hidden","height":"200px"}' class="img-box">
				<img @click.stop="imgPreView(item.jiudiantupian)" :style='{"width":"100%","objectFit":"cover","display":"block","height":"100%"}' v-if="item.jiudiantupian && item.jiudiantupian.substr(0,4)=='http'" :src="item.jiudiantupian.split(',')[0]" class="image" />
				<img @click.stop="imgPreView(baseUrl + (item.jiudiantupian?item.jiudiantupian.split(',')[0]:''))" :style='{"width":"100%","objectFit":"cover","display":"block","height":"100%"}' v-else :src="baseUrl + (item.jiudiantupian?item.jiudiantupian.split(',')[0]:'')" class="image" />
			</div>
		    <div :style='{"padding":"0 10px","overflow":"hidden","flex":"1","display":"flex","height":"auto"}' class="item-info">
		      <div :style='{"width":"50%","padding":"0 10px 0 0","color":"#999","flex":"1"}'>
				...
				<div class="title">{{item.chengshifenlei}}</div>
				<div class="title">{{item.xingji}}</div>
  ...
  export default {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        if (this.formSearch.chengshifenlei != '') searchWhere.chengshifenlei = '%' + this.formSearch.chengshifenlei + '%';
        if (this.formSearch.xingji != '') searchWhere.xingji = '%' + this.formSearch.xingji + '%';
        if (this.timeRange.length > 0) {
          searchWhere.tuifangshijianstart = this.timeRange[0];
          searchWhere.tuifangshijianend = this.timeRange[1];
        }
			let user = JSON.parse(localStorage.getItem('sessionForm'))
		if (this.sortType) searchWhere.sort = this.sortType
		if (this.sortOrder) searchWhere.order = this.sortOrder
        this.$http.get(`jiudiandingdan/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/jiudiandingdanDetail', query: params});
      },
