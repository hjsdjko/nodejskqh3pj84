...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"row-gap":"10px","padding":"24px 0","alignItems":"center","flexWrap":"wrap","display":"flex","width":"100%","height":"auto"}'>
	  ...
	  <el-button v-if="btnAuth('jiudianxinxi','新增')" :style='{"cursor":"pointer","border":"1px solid","padding":"0px 15px","margin":"0 10px 0 0","outline":"none","color":"#000","borderRadius":"4px","background":"none","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/jiudianxinxiAdd')"><i v-if="true" :style='{"color":"#000","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
		  ...
		  <div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
		    ...
		    <div :style='{"padding":"0 10px","overflow":"hidden","flex":"1","display":"flex","height":"auto"}' class="item-info">
		      <div :style='{"width":"50%","padding":"0 10px 0 0","color":"#999","flex":"1"}'>
				...
				<div class="title">{{item.leibie}}</div>
		      ...
		      <div class="desc" v-html="item.jiudianjieshao"></div>
		    </div>
		  </div>
		</div>
	</div>
...
<script>
  export default {
    ...
    created() {
		...
		this.selectOptionsList.push({name:'类别',list:this.leibieOptions,tableName: 'leibie',check: -1})
	    this.xingjiOptions = '一星,二星,三星,四星,五星'.split(',');
		this.selectOptionsList.push({name:'星级',list:this.xingjiOptions,tableName: 'xingji',check: -1})
	    this.fangjianleixingOptions = '单人间,大床房,双人间,三人间,标准套间,豪华套间'.split(',');
		this.selectOptionsList.push({name:'房间类型',list:this.fangjianleixingOptions,tableName: 'fangjianleixing',check: -1})
      this.getFenlei();
      this.getList(1, '全部');
    },
    //方法集合
    methods: {
      ...
      getFenlei() {
		...
		this.feileiColumn = 'chengshifenlei'
      },
      getList(page, fenlei, ref = '') {
        ...
        if (this.curFenlei != '全部') searchWhere.chengshifenlei = this.curFenlei;
